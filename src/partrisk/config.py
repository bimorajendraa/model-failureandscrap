...
# config.py ada di src/partrisk/config.py - repo root (tempat models/ dan
# .env sungguhan tinggal, models/ ARTIFACT bukan bagian package) ada TIGA
# tingkat di atasnya (config.py -> partrisk/ -> src/ -> root). Default ini
# menghitung itu secara struktural, BUKAN menebak - jadi benar otomatis
# selama layout src/partrisk/ ini dipertahankan, tanpa perlu env var apa
# pun di dev biasa. PARTRISK_HOME override tetap ada untuk kasus di mana
# struktur relatif itu TIDAK berlaku (mis. Docker image yang tidak
# menyalin src/ apa adanya - lihat Dockerfile ENV PARTRISK_HOME).
PACKAGE_DIR = Path(os.environ.get("PARTRISK_HOME", str(Path(__file__).resolve().parent.parent.parent)))
